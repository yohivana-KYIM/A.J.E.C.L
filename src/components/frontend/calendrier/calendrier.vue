...
<template>
    ...
    <div class="controls">
      <div class="custom-events-container">
        ...
        <form id="custom-event-form">
          ...
          <input type="text" id="custom-event-description" placeholder="Description de l'événement" required>
          ...
          <input type="number" id="custom-event-frequency" placeholder="Fréquence (en semaines, facultatif)" min="1">
          <input type="tel" id="responsible1-phone" placeholder="WhatsApp Responsable 1 (facultatif)">
          <input type="tel" id="responsible2-phone" placeholder="WhatsApp Responsable 2 (facultatif)">
    ...
    <section id="readme">
      ...
      <p>Bienvenue dans l'application interactive de gestion de calendriers et d'événements. Ce projet permet de visualiser des calendriers mensuels, d'ajouter des événements personnalisés, de filtrer les événements par catégories et de gérer des récurrences complexes.</p>
      ...
      <h3>Contributions</h3>
      <p>Ce projet est open-source. N'hésitez pas à contribuer en proposant des améliorations ou en signalant des problèmes.</p>
  
      <h3>Contact</h3>
      <p>Pour toute question ou demande d'assistance, contactez l'administrateur via email ou WhatsApp.</p>
    </section>
  </template>
  ...
  <script>
    ...
    function notifyResponsibles() {
      ...
      for (const [date, details] of Object.entries(userCustomDays)) {
        ...
        if (message) {
          if (details.responsible1) sendWhatsAppNotification(details.responsible1, message);
          if (details.responsible2) sendWhatsAppNotification(details.responsible2, message);
        }
      }
    }
    ...
    function updateCustomEventsList() {
      ...
      Object.entries(userCustomDays).forEach(([date, details]) => {
        const description = typeof details === "object" ? details.description : details;
    ...
    function addCustomEvent(date, description, frequencyType, frequencyValue, responsible1, responsible2) {
      const occurrences = frequencyType === "weekly"
      ...
      occurrences.forEach(occurrence => {
        userCustomDays[occurrence] = {
          description: `${description} (toutes les ${frequencyValue || 1} ${frequencyType === 'weekly' ? 'semaine(s)' : 'mois'})`,
    ...
    function getTooltip(date) {
      ...
      // Parcourir toutes les sources d'événements
      for (const [category, events] of Object.entries(eventCategories)) {
        if (events[date]) {
          const description = typeof events[date] === "object"
    ...
    function createCalendar(year, month) {
      ...
      while (currentDay <= daysInMonth) {
        ...
        const dayElement = $(`<div class="${dayClasses.join(' ')}">${currentDay}</div>`);
        if (tooltip) {
    ...
    let selectedEventTypes = [
      "holidays", "optimalDays", "worldDays",
      "cameroonDays", "lyonDays", "culturalDays",
      "africanDays", "sportsDays",
      "userCustomDays"
    ];
    ...
    $(document).ready(function () {
      ...
      $("#add-custom-event").on("click", function () {
        ...
        if (date && description && frequencyType) {
          addCustomEvent(date, description, frequencyType, frequencyValue, responsible1, responsible2);
        } else {
      ...
      function validateJson(data) {
        const requiredKeys = [
          "holidays", "optimalDays", "worldDays",
          "cameroonDays", "lyonDays", "culturalDays",
          "africanDays", "sportsDays", "userCustomDays"
        ];
          ...
          reader.onload = function (e) {
            try {
              ...
              if (validateJson(data)) {
                ...
                userCustomDays = {};
                for (const [groupKey, groupDetails] of Object.entries(data.userCustomDays)) {
    ...
    <style>
		...
		@media (max-width: 768px) {
            ...
            #custom-event-form input, #custom-event-form select, #custom-event-form button {
                flex: 1 1 100%;
            }
